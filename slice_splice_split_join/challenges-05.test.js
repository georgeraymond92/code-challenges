...
const listFoods = (recipe) => {
  ...
  for(let i=0; i <= recipe.ingredients.length-1;i++){
      ...
      sliceStart = item.indexOf(' ', firstIndex + 1) + 1;
      result.push(item.slice(sliceStart));
  }
  return result;
}
...
const splitFoods = (recipe) => {
  ...
  for(let i=0; i <= recipe.ingredients.length-1;i++){
    ...
    let joinAt = array.indexOf(" ", firstIndex + 1) + 1;
    let newString = array.slice(joinAt).join("");
...
const stepActions = (recipe) => {
  ...
  for(let i=0; i <= recipe.steps.length-1;i++){
    const item = recipe.steps[i];
    let array = item.split('');
    let sliceAt = array.indexOf(" ");
    let newString = array.slice(0, sliceAt).join("");
    result.push(newString)
    }
  return result;
}
...
const extractVowels = (str) => {
  // Solution code here...
    ...
    stringSplit.forEach(function(value){
        if(value === "a" || value === "e" || value === "i" || value === "o" || value === "u"){
            vowels.push(value);
        }else {
            noVowels.push(value);
    ...
    array.push(noVowels.join(""));
    array.push(vowels.sort().join(""));
    return array;
};
